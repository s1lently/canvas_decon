...
def fetch_assignment_details(session, assignment_url, assignment_name, due_date, base_todo_dir):
    try:
        ...
        print(f"  API: {api_url}"); response = session.get(api_url); response.raise_for_status(); assignment_data = response.json(); description = assignment_data.get('description', ''); file_infos = extract_file_ids(description); downloaded_files = []; folder_name = None; assignment_folder = None
        if file_infos:
            print(f"  Found {len(file_infos)} file(s)"); assignment_folder = create_assignment_folder(base_todo_dir, assignment_name, due_date); folder_name = os.path.basename(assignment_folder); print(f"  Folder: {folder_name}")
            for file_info in file_infos:
                download_url = f"{config.CANVAS_BASE_URL}/courses/{file_info['course_id']}/files/{file_info['file_id']}/download?download_frd=1"; print(f"    Downloading: {download_url}"); result = download_file(session, download_url, assignment_folder)
                if result['success']: print(f"      Saved: {result['filename']} ({result['size']} bytes)"); downloaded_files.append({'file_id': file_info['file_id'], 'download_url': download_url, 'filename': result['filename'], 'local_path': result['path']})
                else: print(f"      Error: {result['error']}"); downloaded_files.append({'file_id': file_info['file_id'], 'download_url': download_url, 'error': result['error']})
        else: print("  No files found"); assignment_folder = create_assignment_folder(base_todo_dir, assignment_name, due_date)
...
def process_and_save_todos(todos, session):
    ...
    for item in todos:
        ...
        print(f"\nFetching: {todo_data['name']}"); print(f"  URL: {redirect_url}"); assignment_details = fetch_assignment_details(session, redirect_url, assignment_name, due_date, todo_dir); todo_data['assignment_details'] = assignment_details
        if 'error' in assignment_details: print(f"  Error: {assignment_details['error']}")
        else: print(f"  Type: {', '.join(assignment_details.get('type', []))}, Quiz: {assignment_details.get('is_quiz')}, Submitted: {assignment_details.get('submitted')}")
